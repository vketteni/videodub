    ...
    try:
        ...
        for model_name, model_data in cost_summary["models"].items():
            ...
            if usage["input_tokens"] > 0:
                print(
                    f"      Input tokens: {usage['input_tokens']:,} (${costs['input_cost']:.6f})"
                )
                print(
                    f"      Output tokens: {usage['output_tokens']:,} (${costs['output_cost']:.6f})"
                )
            if usage["characters"] > 0:
                print(
                    f"      Characters: {usage['characters']:,} (${costs['audio_cost']:.6f})"
                )
...
async def model_comparison_test(
    ...
    for model in models:
        ...
        result = await quick_translation_test(
            video_url=video_url, target_language=target_language, openai_model=model
        )
        if result:
    ...
    print(f"\n📈 Comparison Summary:")
    print(
        f"{'Model':<15} {'Duration':<10} {'Status':<10} {'Total Cost':<12} {'Tokens':<10}"
    )
    print("-" * 70)
    ...
    for result in results:
        ...
        print(
            f"{result['model']:<15} {duration:<10} {status:<10} {cost_str:<12} {tokens_str:<10}"
        )
