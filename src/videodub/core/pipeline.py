...
from .exceptions import PipelineError, TranslationError, TTSError, VideoScrapingError
...
class TranslationPipeline:
    ...
    async def process_video(
        ...
        try:
            ...
            await self._save_pipeline_data(
                video_id, metadata, translated_segments, result
            )
    ...
    async def process_existing_video(
        self, video_id: str, new_target_language: Optional[str] = None
    ) -> ProcessingResult:
        """
        ...
        try:
            ...
            _, transcript_segments = await self.video_scraping_service.get_transcript(
                original_url
            )
            ...
            logger.error(
                "Video reprocessing failed", video_id=video_id, error=error_msg
            )
            result.mark_failed(error_msg)
        ...
        try:
            ...
            for video_id in video_ids:
                try:
                    ...
                    logger.warning(
                        "Failed to get status for video",
                        video_id=video_id,
                        error=str(e),
                    )
                    continue
    ...
    async def _scrape_video(self, url: str) -> tuple[VideoMetadata, List]:
        ...
        try:
            ...
            (
                metadata,
                transcript_segments,
            ) = await self.video_scraping_service.scrape_video(url)
    ...
    async def _translate_segments(
        self, video_id: str, segments: List, target_language: str
    ) -> List:
        ...
        try:
            ...
            job = TranslationJob(
                ...
                target_language=target_language,
                tts_engine=self.config.tts_engine,
                output_directory=await self.storage_service.get_video_directory(
                    video_id
                ),
            )
            ...
            # Translate segments
            translated_segments = []
            async for translated_segment in self.translation_service.translate_batch(
                job
            ):
    ...
    async def _generate_audio(
        self, video_id: str, segments: List, language: str
    ) -> List[Path]:
    ...
    async def _create_dubbed_video(
        self, video_id: str, metadata: VideoMetadata, segments: List
    ) -> Optional[Path]:
        """Create dubbed video by combining original video with translated audio."""
        try:
            logger.info(
                "Starting video dubbing", video_id=video_id, segments=len(segments)
            )
