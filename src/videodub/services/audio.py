...
class FFmpegAudioProcessingService(AudioProcessingService):
    ...
    async def combine_audio_segments(
        ...
        if len(existing_files) != len(audio_files):
            logger.warning(
                f"Some audio files missing: {len(audio_files) - len(existing_files)} files not found"
            )

        logger.info(
            f"Combining {len(existing_files)} audio segments into {output_path}"
        )
        ...
        try:
            ...
            if not output_path.exists():
                raise PipelineError(
                    f"Combined audio file was not created: {output_path}"
                )
    ...
    async def adjust_audio_timing(
        ...
        # Create output path
        output_path = (
            audio_path.parent / f"{audio_path.stem}_adjusted{audio_path.suffix}"
        )
        ...
        try:
            ...
            cmd = [
                ...
                str(audio_path),
                "-af",
                f"atempo={target_duration}",  # Adjust tempo to match duration
                "-y",  # Overwrite output file
                str(output_path),
            ]
...
class PydubAudioProcessingService(AudioProcessingService):
    ...
    def __init__(self):
        ...
        try:
            ...
            raise PipelineError(
                "pydub is not installed. Please install it or use FFmpeg."
            )
