...
class FFmpegVideoProcessingService(VideoProcessingService):
    ...
    async def create_dubbed_video(
        ...
        if not translated_audio_path.exists():
            raise PipelineError(
                f"Translated audio file not found: {translated_audio_path}"
            )
        ...
        logger.info(
            f"Creating dubbed video: {original_video_path} + {translated_audio_path} -> {output_path}"
        )
        ...
        try:
            ...
            cmd = [
                "ffmpeg",
                "-i",
                str(original_video_path),  # Input video
                "-i",
                str(translated_audio_path),  # Input audio
                "-c:v",
            ...
            logger.info(
                f"Successfully created dubbed video: {output_path} ({file_size:,} bytes)"
            )
            return output_path
    ...
    async def extract_video_metadata(self, video_path: Path) -> Dict:
        ...
        try:
            ...
            logger.info(
                f"Extracted metadata: {video_metadata['duration']:.2f}s, {video_metadata.get('width', 0)}x{video_metadata.get('height', 0)}"
            )
...
class BasicVideoProcessor(VideoProcessingService):
    ...
    async def create_dubbed_video(
        ...
        logger.warning(
            "BasicVideoProcessor: No actual dubbing performed, copying original video"
        )
...
def create_video_processor() -> VideoProcessingService:
    ...
    try:
        ...
        logger.warning(
            "FFmpeg not available for video processing, using basic processor"
        )
