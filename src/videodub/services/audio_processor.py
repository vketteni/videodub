...
class FFmpegAudioProcessor(AudioProcessor):
    """Audio processor using FFmpeg for audio manipulation."""
...
class PydubAudioProcessor(AudioProcessor):
    """Fallback audio processor using pydub when FFmpeg is not available."""
...
def create_audio_processor() -> AudioProcessor:
    """
    Factory function to create the best available audio processor.
    
    Returns:
        AudioProcessor instance (FFmpeg preferred, pydub fallback)
    """
    try:
        ...
        try:
            return PydubAudioProcessor()
        except PipelineError:
            raise PipelineError(
                "No audio processor available. Please install FFmpeg or pydub."
            )