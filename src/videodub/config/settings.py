...
class PipelineSettings(BaseSettings):
    ...
    google_credentials_path: Optional[str] = Field(
        None, description="Google Cloud credentials file path"
    )
    azure_speech_key: Optional[str] = Field(
        None, description="Azure Speech Services key"
    )
    azure_speech_region: str = Field(
        "eastus", description="Azure Speech Services region"
    )
    azure_translator_key: Optional[str] = Field(
        None, description="Azure Translator key"
    )
    azure_translator_region: Optional[str] = Field(
        None, description="Azure Translator region"
    )

    # Processing Configuration
    default_target_language: str = Field(
        "es", description="Default target language code"
    )
    default_tts_engine: TTSEngine = Field(
        TTSEngine.OPENAI, description="Default TTS engine"
    )
    max_concurrent_requests: int = Field(
        5, ge=1, le=20, description="Maximum concurrent API requests"
    )
    request_timeout: int = Field(
        30, ge=5, le=300, description="Request timeout in seconds"
    )
    retry_attempts: int = Field(
        3, ge=0, le=10, description="Number of retry attempts for failed requests"
    )

    # Storage Configuration
    output_directory: str = Field(
        "./output", description="Output directory for processed files"
    )
    temp_directory: Optional[str] = Field(
        None, description="Temporary directory for processing"
    )
    keep_temp_files: bool = Field(
        False, description="Whether to keep temporary files after processing"
    )
    ...
    extract_audio: bool = Field(
        True, description="Whether to extract audio from videos"
    )
    extract_transcript: bool = Field(True, description="Whether to extract transcripts")
    max_video_duration: int = Field(
        3600, ge=60, description="Maximum video duration in seconds"
    )
    ...
    enable_structured_logging: bool = Field(
        True, description="Enable structured JSON logging"
    )
    ...
    enable_health_checks: bool = Field(
        True, description="Enable health check endpoints"
    )
    ...
    def validate_language_code(cls, v: str) -> str:
        ...
        if len(v) != 2 or not v.islower():
            raise ValueError(
                'Language code must be a 2-letter lowercase code (e.g., "es", "fr")'
            )
        return v
    ...
    def validate_api_keys(
        self, required_engines: Optional[Set[TTSEngine]] = None
    ) -> List[str]:
        """
...
class LanguageConfig:
    ...
    def get_language_name(cls, language_code: str) -> str:
        ...
        return cls.SUPPORTED_LANGUAGES.get(language_code, {}).get(
