...
def validate_api_key(api_key: str, key_type: str) -> bool:
    ...
    if key_type.lower() == "openai":
        ...
        if not re.match("^[a-f0-9]+$", api_key):
            raise ValidationError("Azure API key must contain only hexadecimal characters")
...
def validate_audio_format(audio_format: str, supported_formats: Optional[List[str]] = None) -> bool:
    """
