...
MODEL_PRICING = {
    ...
    OpenAIModel.GPT_4_1_NANO: ModelPricing(0.0001, 0.0003),  # Estimated - adjust when available
...
class CostTracker:
    ...
    def calculate_cost(self, model_name: str) -> CostBreakdown:
        ...
        if model_name not in self.usage_by_model:
            return CostBreakdown(
                model=model_name, input_tokens=0, output_tokens=0, characters=0,
                input_cost=0.0, output_cost=0.0, audio_cost=0.0, total_cost=0.0, requests=0
            )
    ...
    def get_session_summary(self) -> Dict:
        ...
        for model_name in self.usage_by_model:
            ...
            summary["totals"]["total_tokens"] += cost_breakdown.input_tokens + cost_breakdown.output_tokens
