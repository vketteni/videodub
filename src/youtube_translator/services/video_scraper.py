...
from ..core.models import VideoMetadata, TranscriptSegment
...
class YouTubeScrapingService(VideoScrapingService):
    ...
    async def scrape_audio_only(self, url: str) -> Tuple[VideoMetadata, Path]:
        ...
        try:
            ...
            metadata = VideoMetadata(
                video_id=getattr(result, 'video_id', None) or self._extract_video_id_from_url(url),
            ...
            # Get audio file path
            audio_path = Path(getattr(result, 'file_path', None) or getattr(result, 'audio_path', None))
    ...
    def _extract_metadata(self, url: str, result: Any) -> VideoMetadata:
        ...
        return VideoMetadata(
            video_id=getattr(result, 'video_id', None) or 
                     getattr(result.metadata, 'video_id', None) or
                     self._extract_video_id_from_url(url),
            title=result.metadata.title,
            duration=result.metadata.duration,
            url=url,
            channel=getattr(result.metadata, 'channel', None),
    ...
    def _try_load_transcript_from_file(self, result: Any) -> List[TranscriptSegment]:
        ...
        try:
            ...
            logger.warning(
                "Could not load transcript from file",
                error=str(e)
            )
