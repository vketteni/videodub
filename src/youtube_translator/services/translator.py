...
class OpenAITranslationService(TranslationService):
    ...
    async def translate_reconstructed_segments(
        self, 
        segments: List[TranscriptSegment], 
        target_language: str
    ) -> List[TranslationSegment]:
        """
        Translate segments using sentence reconstruction for better quality.
        
        This method:
        1. Reconstructs complete sentences from fragmented segments
        2. Translates full sentences for better context and quality
        3. Maps timing information back to original segments
        ...
        Returns:
            List of translated segments with reconstructed timing
        ...
        logger.info(
            "Starting sentence-based translation",
            target_language=target_language,
            segment_count=len(segments)
        )
        
        # Initialize transcript processor
        processor = TranscriptProcessor(min_sentence_length=15, max_sentence_length=300)
        
        try:
            # Reconstruct sentences from segments
            sentence_segments, timing_maps = processor.process_transcript(segments)
            
            logger.info(
                "Transcript reconstructed",
                original_segments=len(segments),
                reconstructed_sentences=len(sentence_segments)
            )
            ...
            for sentence_segment in sentence_segments:
                try:
                    translated_text = await self.translate_text(
                        sentence_segment.sentence_text, 
                        target_language
                    )
                    translated_sentences.append({
                        'original': sentence_segment,
                        'translated': translated_text
                    })
                    
                    logger.debug(
                        "Sentence translated",
                        original_length=len(sentence_segment.sentence_text),
                        translated_length=len(translated_text),
                        confidence=sentence_segment.confidence_score
                    )
                    
                except Exception as e:
                    logger.error(
                        "Failed to translate sentence",
                        sentence=sentence_segment.sentence_text[:100],
                        error=str(e)
                    )
                    # Use fallback
                    translated_sentences.append({
                        'original': sentence_segment,
                        'translated': f"[Translation failed: {sentence_segment.sentence_text}]"
                    })
            
            # Create one TranslationSegment per sentence (not per original segment)
            result_segments = []
            
            for sentence_idx, translated_sentence_data in enumerate(translated_sentences):
                translated_text = translated_sentence_data['translated']
                original_sentence = translated_sentence_data['original']
                
                # Create a synthetic segment representing the complete sentence
                # Use the first source segment as the base, but with sentence timing
                first_source_segment = original_sentence.source_segments[0] if original_sentence.source_segments else None
                
                if first_source_segment:
                    # Create a sentence-level translation segment
                    sentence_translation_segment = TranslationSegment(
                        original_segment=first_source_segment,  # Reference segment
                        translated_text=translated_text,
                        language=target_language
                    )
                    
                    # Add comprehensive metadata for audio generation and timing reconstruction
                    sentence_translation_segment.sentence_context = {
                        'sentence_index': sentence_idx,
                        'sentence_start': original_sentence.start_time,
                        'sentence_end': original_sentence.end_time,
                        'sentence_duration': original_sentence.duration,
                        'source_segments': [
                            {
                                'start_time': seg.start_time,
                                'end_time': seg.end_time,
                                'original_text': seg.text
                            } for seg in original_sentence.source_segments
                        ],
                        'confidence_score': original_sentence.confidence_score,
                        'is_sentence_level': True  # Flag to indicate this is a sentence-level segment
                    }
                    
                    result_segments.append(sentence_translation_segment)
            
            successful_translations = len([
                seg for seg in result_segments 
                if not seg.translated_text.startswith("[Translation failed:")
            ])
            
            logger.info(
                "Sentence-based translation completed",
                target_language=target_language,
                original_segments=len(segments),
                reconstructed_sentences=len(sentence_segments),
                translation_segments_created=len(result_segments),
                successful_translations=successful_translations,
                failed_translations=len(result_segments) - successful_translations
            )
            
            return result_segments
            
        except Exception as e:
            logger.error("Sentence reconstruction failed, falling back to segment translation", error=str(e))
            # Fallback to original segment-based translation
            return await self.translate_segments(segments, target_language)
