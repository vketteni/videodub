...
class TestTranslationPipeline:
    """Test TranslationPipeline implementation."""
    ...
    async def test_process_video_with_alignment_comparison(
        ...
        results = await pipeline.process_video_with_alignment_comparison(
    ...
    async def test_extract_data(
        self, pipeline, mock_services, sample_timed_transcript
    ):
        ...
        timed_transcript = await pipeline._extract_data(
    ...
    async def test_translate_texts(
        self, pipeline, mock_services, sample_timed_transcript, sample_translated_texts
    ):
        ...
        translated_texts = await pipeline._translate_texts(
    ...
    async def test_align_translation(
        ...
        timed_translation = await pipeline._align_translation(
    ...
    def test_extract_video_id(self, pipeline):
        """Test video ID extraction."""
        # YouTube URL with v= parameter
        video_id = pipeline._extract_video_id(
            "https://youtube.com/watch?v=abc123&t=30s"
        )
        assert video_id == "abc123"

        # YouTube short URL
        video_id = pipeline._extract_video_id("https://youtu.be/xyz789")
        assert video_id == "xyz789"

        # Non-YouTube URL (fallback)
        video_id = pipeline._extract_video_id("https://example.com/video")
